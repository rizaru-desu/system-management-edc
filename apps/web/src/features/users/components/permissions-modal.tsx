...
export function PermissionsModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="theme-light max-h-[85vh] gap-0 overflow-hidden border-[#DDE0EC] bg-white p-0 text-[#0E2748] sm:max-w-2xl">
        <DialogHeader className="border-b border-[#DDE0EC] p-6 pb-4">
          ...
          <DialogDescription className="text-[#0E2748]/60">
            Effective module access from the assigned roles.
          </DialogDescription>
          <div className="flex flex-wrap items-center gap-1.5 pt-1">
            {user.roles.map((role) => (
              <RoleBadge key={role} role={role} />
            ))}
            <span className="ml-auto text-xs font-medium text-[#3F6FA8]">
        ...
        <p className="border-t border-[#DDE0EC] bg-[#F6F7F9] px-6 py-3 text-[11px] text-[#0E2748]/50">
