...
export function UserFormModal({
  ...
  const [initialValues, setInitialValues] = useState<UserFormValues>(EMPTY)

  // Re-seed the form whenever the modal opens for a different target. The
  // auto-generated password is part of the seed, so an untouched create
  // form counts as clean.
  useEffect(() => {
    if (open) {
      const seeded = user
        ? {
              ...
              {isSysAdmin && (
                <p className="flex items-start gap-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800">
                  <ShieldAlert
                    ...
                    strokeWidth={2}
                  />
                  System Administrator already grants full access, so it cannot
                  be combined with other roles. Unselect it to mix roles.
                </p>
              )}
              {errors.roles && (
