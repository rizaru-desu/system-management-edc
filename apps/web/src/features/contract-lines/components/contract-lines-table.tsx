...
interface ContractLinesTableProps {
  ...
  isFiltering: boolean
  onClearFilters: () => void
  onEdit: (record: ContractLineRecord) => void
  onStatusToggle: (record: ContractLineRecord) => void
  onDelete: (record: ContractLineRecord) => void
}
...
export function ContractLinesTable({
  ...
  const columns = useMemo<Array<LegacyColumnDef<ContractLineRecord>>>(
    () => [
      {
        id: 'lineNumber',
        header: 'Line Number',
      ...
      },
      {
        ...
        header: 'Line Name',
        cell: ({ row }) => (
          <span className="block max-w-xs truncate font-medium text-brand-900">
            {row.original.name}
          </span>
        ),
      },
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Badge variant="soft">
            {documentStatusLabel(row.original.documentStatus)}
          </Badge>
        ),
      },
      {
              ...
              <td colSpan={columns.length} className="px-5">
                {isFiltering ? (
                  ...
                  <EmptyState
                    ...
                    description="Add the first contract line to start managing the catalogue."
                  />
                )}
              </td>
            </tr>
