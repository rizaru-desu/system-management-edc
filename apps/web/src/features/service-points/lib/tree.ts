...
export interface ServicePointNode {
  record: ServicePointRecord
  children: Array<ServicePointNode>
}

/** A tree node flattened for table rendering, with its display metadata. */
export interface ServicePointRow {
  record: ServicePointRecord
  /** 0 = top level; drives the indentation of the tree column. */
  depth: number
 ...
 * preserved. Records pointing at a missing parent are promoted to roots so a
 * broken reference can never silently drop a whole subtree from the table.
 */
export function buildServicePointTree(
  records: Array<ServicePointRecord>,
): Array<ServicePointNode> {
  const nodes = new Map<string, ServicePointNode>(
    records.map((record) => [record.id, { record, children: [] }]),
  )
  const roots: Array<ServicePointNode> = []
...
/**
 * Prunes the tree down to records matching `predicate`, keeping the ancestor
 * chain of every match so the hierarchy context stays visible (a hit on
 * "Kebayoran Baru" still renders under Jakarta → Jakarta Selatan).
 */
export function filterServicePointTree(
  roots: Array<ServicePointNode>,
  predicate: (record: ServicePointRecord) => boolean,
): Array<ServicePointNode> {
  const prune = (node: ServicePointNode): ServicePointNode | null => {
    const children = node.children
      .map(prune)
      .filter((child): child is ServicePointNode => child !== null)
    if (children.length === 0 && !predicate(node.record)) return null
    return { record: node.record, children }
  }
  return roots
    .map(prune)
    .filter((node): node is ServicePointNode => node !== null)
}
...
): Array<ServicePointRow> {
  const rows: Array<ServicePointRow> = []
  const walk = (
...
/**
 * Parent choices for the add/edit dialog, in tree (depth-first) order with
 * their depth for indentation. When editing, the record itself and its whole
 * subtree are excluded — a service point cannot be its own ancestor.
 */
export function buildParentOptions(
  ...
  const walk = (node: ServicePointNode, depth: number) => {
    if (!excluded.has(node.record.id)) {
      options.push({
        id: node.record.id,
        label: `${node.record.name} (${node.record.code})`,
        depth,
      })
      for (const child of node.children) walk(child, depth + 1)
    }
  }
  for (const root of buildServicePointTree(records)) walk(root, 0)
  return options
}
