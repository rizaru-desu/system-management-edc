  ...
  try {
    ...
    for (const name of Object.keys(interfaces)) {
      ...
      for (const net of netList) {
        ...
        const isIpv4 =
          net.family === 'IPv4' ||
          (net as unknown as { family: number }).family === 4;
...
export function resolveTrustedOrigins(
  ...
  const authOrigin = getOriginFromUrl(
    options.betterAuthUrl !== undefined
...
export function getCorsOriginDelegate(
  ...
  return (origin, callback) => {
    if (isOriginAllowed(origin, trustedOrigins)) {
      ...
      callback(
        new Error(
          `CORS error: Origin ${origin} is not allowed by trusted origins.`,
        ),
      );
    }
  };
