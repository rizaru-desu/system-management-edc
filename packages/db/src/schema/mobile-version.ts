 ...
 */
export const mobileVersion = pgTable(
  "mobile_version",
  {
    id: text("id")
      .primaryKey()
  ...
  (table) => [
    // One release per (platform, update type, version name, version code) —
    // the database backstop behind the application-level duplicate check.
    uniqueIndex("mobile_version_release_identity_idx").on(
      table.platform,
      table.updateType,
      table.latestVersion,
      table.versionCode,
